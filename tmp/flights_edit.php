				...
				<div class="found_form parentbox">	
					<form method="POST" action="flights.php">		
						<div class="info">
							...
							<ul>								
								<li>Узнать детали конкретного рейса: его статус, время посадки и вылета самолёта. Для этого задайте дату (по умолчанию стоит сегодня) и укажите номер рейса.</li>								
						...
						<div class="inline_block owerflow_hide">
							<h3>Дата</h3>
							<input type="date" name="flight_date[0]" class="input date_from width_1" min='$today' id="date_from" value="$flight_date[0]" required>
						</div>						
						<div class="clear_both"></div>			
						<div>
					...
					{						
						...
						{				
							...
							$query[0]  = "SELECT DISTINCT schedule.flight_date, 
														...
														schedule.flight_date = '$flight_date[0]' AND
														flights.flight_number LIKE '%$flight_number[0]%'
														ORDER BY time_up";
